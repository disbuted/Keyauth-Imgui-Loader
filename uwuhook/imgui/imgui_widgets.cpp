...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    window->DrawList->AddRectFilledMultiColor( bb.Min, bb.Max, ( hovered && held ) ? GetColorU32( ImGuiCol_ButtonActive ) : hovered ? GetColorU32( ImGuiCol_ButtonHovered ) : GetColorU32( ImGuiCol_Button ), GetColorU32( ImGuiCol_FrameBg ), GetColorU32( ImGuiCol_FrameBg2 ), GetColorU32( ImGuiCol_FrameBg2 ) );
    window->DrawList->AddRect( bb.Min, bb.Max, ( hovered && held ) ? GetColorU32( ImGuiCol_Scheme ) : GetColorU32( ImGuiCol_BorderShadow ) );
    window->DrawList->AddText( bb.GetCenter( ) - label_size / 2, ( hovered && held ) ? GetColorU32( ImGuiCol_Scheme ) : hovered ? GetColorU32( ImGuiCol_Text ) : GetColorU32( ImGuiCol_TextDisabled ), label, FindRenderedTextEnd( label ) );
...
bool ImGui::Checkbox( const char* label, bool* v ) {
    ...
    ImVec2 label_size = text_size( font, 13, label );

    const float square_sz = 9.f;
    ImRect bb( pos, pos + ImVec2( square_sz, square_sz ) );
    ImRect total_bb( pos, pos + ImVec2( square_sz + label_size.x + GImGui->Style.ItemInnerSpacing.x, square_sz ) );
    ...
    draw->AddRectFilled( bb.Min, bb.Max, GetColorU32( ImGuiCol_FrameBg ) );
    if ( hovered || ( *v ) ) draw->AddRectFilledMultiColor( bb.Min, bb.Max, GetColorU32( ImGuiCol_Scheme, hovered && !( *v ) ? 0.25f : ( *v ) ? 1.f : 0.f ), GetColorU32( ImGuiCol_Scheme, hovered && !( *v ) ? 0.25f : ( *v ) ? 1.f : 0.f ), GetColorU32( ImGuiCol_Scheme2, hovered && !( *v ) ? 0.25f : ( *v ) ? 1.f : 0.f ), GetColorU32( ImGuiCol_Scheme2, hovered && !( *v ) ? 0.25f : ( *v ) ? 1.f : 0.f ) );
    draw->AddRect( bb.Min, bb.Max, GetColorU32( ImGuiCol_BorderShadow ) );
    add_text( font, 13, { total_bb.Max.x - label_size.x, bb.GetCenter( ).y - label_size.y / 2 - 1 }, ( *v ) ? GetColorU32( ImGuiCol_Text ) : hovered ? GetColorU32( ImGuiCol_Text, 0.5f ) : GetColorU32( ImGuiCol_TextDisabled ), label, FindRenderedTextEnd( label ) );
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline) // Open group before calling GetID() because groups tracks id created within their scope (including the scrollbar)
        BeginGroup();
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = text_size(font, 13, label);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), frame_size.y);

    const ImRect frame_bb({ window->DC.CursorPos.x, window->DC.CursorPos.y + ( label_size.x > 0 ? label_size.y + style.ItemInnerSpacing.y : 0.f ) }, { window->DC.CursorPos.x + frame_size.x, window->DC.CursorPos.y + frame_size.y + ( label_size.x > 0 ? label_size.y + style.ItemInnerSpacing.y : 0.f ) });
    const ImRect total_bb(window->DC.CursorPos, frame_bb.Max);

    ImGuiWindow* draw_window = window;
    ImVec2 inner_size = { frame_size.x - 2, frame_size.y };
    ...
    if (!is_multiline)
    {
        window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, GetColorU32( hovered || IsItemActive( ) ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg ) );
        window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, GetColorU32( ImGuiCol_BorderShadow ) );
    }

    const ImVec4 clip_rect(frame_bb.Min.x + 1, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ...
    if ( label_size.x > 0 )
        add_text( font, 13, { frame_bb.GetCenter( ).x - label_size.x / 2, total_bb.Min.y }, GetColorU32( hovered || IsItemActive( ) ? ImGuiCol_Text : ImGuiCol_TextDisabled ), label, FindRenderedTextEnd( label ) );
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Render
    if ( selected ) {
        window->DrawList->AddRectFilledMultiColor( bb.Min, bb.Max, GetColorU32( ImGuiCol_Border, 0.5f ), GetColorU32( ImGuiCol_Border, 0.f ), GetColorU32( ImGuiCol_Border, 0.f ), GetColorU32( ImGuiCol_Border, 0.5f ) );
        window->DrawList->AddLine( bb.Min, { bb.Min.x, bb.Max.y }, GetColorU32( ImGuiCol_Scheme ) );
    }
    ...
    add_text( font, 13, { bb.Min.x + 11, bb.GetCenter( ).y - 13 / 2 - 1 }, GetColorU32( selected || hovered ? ImGuiCol_Text : ImGuiCol_TextDisabled, hovered && !selected ? 0.5f : 1.f ), label, FindRenderedTextEnd( label ) );
